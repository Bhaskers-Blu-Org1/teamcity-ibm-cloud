...
<table class="runnerFormTable addImageContainerTable">
	...
	<tr>
		...
		<td><select id="${cons.vsiTemplateList}" class="longField"
			data-id="${cons.vsiTemplateList}" name="prop:${cons.vsiTemplateList}" required>
				<props:option value="">Select Image...</props:option>
				<c:forEach var="vsiTemplate" items="${vsiTemplateList}">
					<props:option
						selected="${vsiTemplate.key eq propertiesBean.properties[cons.vsiTemplateList]}"
						value="${vsiTemplate.key}">
						<c:out value="${vsiTemplate.value}" />
					</props:option>
				</c:forEach>
		</select></td>
	</tr>
